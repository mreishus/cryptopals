#!/usr/bin/env python
from pure_python_crypto import sha1_compress, MD4, md_pad_64, little_endian_bytes
...
def generate_mac(message):
    """a secret-prefix MAC: MD4(key || message)"""
    key = mykey()
    # return sha1(key + message)
    return bytes(MD4(key + message))
...
def pad(message, fake_byte_len=None):
    "computes the MD padding of an arbitrary message: Same as MD4"
    length_to_bytes = lambda length: little_endian_bytes([length], 8)
...
def main():
    ...
    # Split md4 into 5 sections of 8 hex characters, convert to decimal
    # "46822a0c"  "b9ef1f48"  "931a14ec" "defe6e1e"

    ## Hexstring -> Big endian integer
    # a = int(mac[0:8], 16)
    # b = int(mac[8:16], 16)
    # c = int(mac[16:24], 16)
    # d = int(mac[24:32], 16)

    ## Hexstring -> Little endian integer
    a = unpack("<I", unhexlify(mac[0:8]))[0]
    b = unpack("<I", unhexlify(mac[8:16]))[0]
    c = unpack("<I", unhexlify(mac[16:24]))[0]
    d = unpack("<I", unhexlify(mac[24:32]))[0]
    ...
    print(f"mac       = {mac}")
    print("")
    print("Split md4 into 4 sections of 8 hex characters, convert to decimal")
    print("a,b,c,d = ", end="")
    print(a, b, c, d)
    print("We will use these values as internal state of MD4 in our extension attack")
    ...
    for keylen in range(20):
        ...
        ## The length of this would be the internal state length of MD4 when it was done
        ...
        ## Generate the forged mac by only MD4 encoding what we want to add, but providing the internal state
